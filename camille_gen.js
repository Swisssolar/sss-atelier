...
function buildOffre(data){
  ...
  if(!Array.isArray(data.avantages) || data.avantages.length < 2 || data.avantages.length > 4) throw new Error('AVANTAGES ABSENTS OU INVALIDES : 2 a 4 benefices specifiques au projet requis (jamais generiques)');
  if(!data.conclusion || String(data.conclusion).trim().length < 40) throw new Error('CONCLUSION PERSONNALISEE ABSENTE : chaque offre doit avoir une conclusion adaptee au projet reel');
  ...
  y-=4;
  // ---------- VOS AVANTAGES (résumé personnalisé du projet) ----------
  ensure(24 + data.avantages.length*12);
  text(ML, y, 'Les avantages de cette solution', 9.5, {bold:true}); y-=13;
  for(const av of data.avantages){ for(const l of wrap('\u2022  '+av, 8.5, false, CW_)){ ensure(12); text(ML+4, y, l, 8.5); y-=11.5; } }
  y-=10;
  // ---------- TABLEAU ----------
  ...
  // Bloc financier INSECABLE : lignes de totaux + bandeau TTC (+ recap) restent ensemble
  const nbLignesTot = 3 + ((ch.remise && Math.abs(ch.remise)>0.004)?1:0);
  const hBloc = nbLignesTot*15 + 34 + (data.recap_ligne?16:0) + 6;
  if(y - hBloc < FOOT_H+30){ newPage(); }
  ...
  // bandeau TTC (espace garanti par le bloc insecable)
  ...
  for(const l of wrap(String(data.conclusion), 8.5, false, CW_)){ ensure(12); text(ML, y, l, 8.5); y-=11.5; }
  y-=10; ensure(50);
  text(ML, y, 'Cordialement,', 8.5); y-=16;
  text(ML, y, 'Adson Bailly', 9, {bold:true}); y-=12;
  text(ML, y, 'Swiss Solar System Sàrl', 8.5); y-=12;
  // (tracabilite des prix conservee uniquement dans les metadonnees — jamais dans le PDF client)
  pages.push(ops);
...
const CAMILLE_VERSION = '2026-07-18-2';
